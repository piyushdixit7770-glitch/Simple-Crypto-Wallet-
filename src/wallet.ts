// Wallet construction + sync-state restore for simple-crypto-wallet.
// Mirrors demo----1/src/wallet.ts — adapted for this project's structure.
  ...
  const wallet = await WalletFacade.init({
    ...
    shielded: async (config) => {
      ...
      if (saved.shielded !== undefined) {
        try {
          const w = await (cls as any).restore(saved.shielded);
          restored.shielded = true;
          return w;
        } catch (err) { warnRestoreFailure('shielded', err); }
      }
    ...
    unshielded: async (config) => {
      ...
      if (saved.unshielded !== undefined) {
        try {
          const w = await (cls as any).restore(saved.unshielded);
          restored.unshielded = true;
          return w;
        } catch (err) { warnRestoreFailure('unshielded', err); }
      }
    ...
    dust: async (config) => {
      ...
      if (saved.dust !== undefined) {
        try {
          const w = await (cls as any).restore(saved.dust);
          restored.dust = true;
          return w;
        } catch (err) { warnRestoreFailure('dust', err); }
      }
